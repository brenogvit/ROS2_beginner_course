...
my_robot_interfaces__msg__LedPanelState__are_equal(const my_robot_interfaces__msg__LedPanelState * lhs, const my_robot_interfaces__msg__LedPanelState * rhs)
{
  ...
  // is_led1_on
  if (lhs->is_led1_on != rhs->is_led1_on) {
    return false;
  }
  // is_led2_on
  if (lhs->is_led2_on != rhs->is_led2_on) {
    return false;
  }
  // is_led3_on
  if (lhs->is_led3_on != rhs->is_led3_on) {
    return false;
  }
  return true;
}
  ...
  my_robot_interfaces__msg__LedPanelState * output)
{
  ...
  // is_led1_on
  output->is_led1_on = input->is_led1_on;
  // is_led2_on
  output->is_led2_on = input->is_led2_on;
  // is_led3_on
  output->is_led3_on = input->is_led3_on;
  return true;
}
