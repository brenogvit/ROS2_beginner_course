...
class TurtleControllerChooseNode(Node):
    ...
    def select_next_target(self):
        ...
        if self.target_index is None or self.target_index >= len(self.alive_turtle_name):
            self.target_index = 0

        self.target_reached = False
        self.get_logger().info(f"Target turtle: {self.alive_turtle_name[self.target_index]}")
        self.target_name = self.alive_turtle_name[self.target_index]
        self.target_x = self.alive_turtle_x[self.target_index]
        self.target_y = self.alive_turtle_y[self.target_index]
        ...
        if self.target_name in self.alive_turtle_name:
            ...
            if self.target_index is not None:
                if index < self.target_index:
                    ...
                    self.target_index = min(self.target_index, len(self.alive_turtle_name) - 1) if self.alive_turtle_name else None
            if not self.alive_turtle_name:
                self.target_name = None
                self.target_x = None
                self.target_y = None
        ...
        if self.current_pose is None or self.target_x is None or self.target_y is None:
            ...
            if self.target_reached or distance < 0.30:
                if self.catch_closest_turtle_first_:
                    ...
                    response.success = True
                    response.name = self.target_name if self.target_name is not None else ""
                    self.get_logger().info(f"Turtle {response.name} caught!")
