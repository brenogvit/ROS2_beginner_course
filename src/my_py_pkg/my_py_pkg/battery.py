    ...
    def check_battery_state(self):
        ...
        if self.battery_state_ == "full":
            if time_now - self.last_time_battery_state_changed_ > 4.0:
                ...
                self.call_set_led(led_number=3, state=True)
                self.call_set_led(led_number=1, state=False)
            ...
            if time_now - self.last_time_battery_state_changed_ > 6.0:
                ...
                self.call_set_led(led_number=3, state=False)
                self.call_set_led(led_number=1, state=True)
